...
def _enabled() -> bool:
    return os.environ.get("HERMES_DREAMING", "").strip().lower() in ("1", "true", "yes")
...
def _handle_slash(raw_args: str, ctx=None) -> str:
    ...
    if sub == "run":
        try:
            result = _schedule.dream_run(force=True, preview=False, respect_quiet=False, scan_recent=True, llm=llm)
    ...
    if sub == "preview":
        try:
            result = _schedule.dream_run(force=True, preview=True, respect_quiet=False, scan_recent=True, llm=llm)
            return _format_result(result)
        except Exception as e:
...
def register(ctx) -> None:
    if not _enabled():
        ctx.register_command(
            ...
            description="Automatic nightly memory consolidation (disabled — set HERMES_DREAMING=1).",
        )
        return
