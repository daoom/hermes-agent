...
| `HERMES_DREAM_PROMOTION_MODE` | `off` | `off` \| `shadow` \| `auto`. Empty, unknown, or malformed values fail closed to `off` |
| `HERMES_DREAM_EXTRACT_PROVIDER` | _(unset)_ | Provider for the extractor route; unset fails closed |
| `HERMES_DREAM_EXTRACT_MODEL` | _(unset)_ | Model for the extractor route; unset fails closed |
| `HERMES_DREAM_VERIFY_PROVIDER` | _(unset)_ | Provider for the verifier route; unset fails closed |
| `HERMES_DREAM_VERIFY_MODEL` | _(unset)_ | Model for the verifier route; unset fails closed |
| `HERMES_DREAM_REM_MODE` | `auto` | `auto` uses `ctx.llm` when available; `off` always uses deterministic fallback |
| `HERMES_DREAM_PROVIDER` | _(unset)_ | Optional Hermes provider override for REM narration only; gated by `plugins.entries.dreaming.llm.allow_provider_override` |
| `HERMES_DREAM_MODEL` | _(unset)_ | Optional Hermes model override for REM narration only; gated by `plugins.entries.dreaming.llm.allow_model_override` |
| `HERMES_DREAM_LLM_TIMEOUT` | `60` | Bounded per-call timeout in seconds, subject to a hard maximum of 300 seconds. Empty, malformed, non-finite (`inf`, `nan`), non-positive, or over-maximum values resolve to the `60` default |
...
| `HERMES_DREAM_MIN_SCORE` | `0.72` | Minimum calibrated score after policy gates. Values in `[0.0, 1.0]` are honoured exactly; blank, malformed, non-finite, negative, or above-one values are **not** clamped but instead close the gate — the resolved threshold **fails closed to no promotion** for the cycle |
| `HERMES_DREAM_MAX_PROMOTIONS` | `1` | Requested automatic promotions per profile and cycle; the pilot enforces a hard upper bound of one. Values above one are hard-clamped to one, negative is zero, and a blank or malformed value **fails closed to zero** rather than silently restoring one |
...
Interactive `/dream run` and `/dream preview` calls use the host plugin LLM facade, so the configured extractor and verifier provider/model overrides must also be allowed by `plugins.entries.dreaming.llm`. Scheduled cycles use the host auxiliary client directly but enforce the same semantic route and runtime-identity checks.

### Modes

- **`off`** — no semantic calls at all and no automatic writes. Every candidate is review-only with reason `promotion_mode_off`. This is the default and the fallback for any unrecognized value.
...
`/dream preview` is non-mutating in every mode: it never writes memory, never clears staging, and never advances the cycle cursor. A non-preview `shadow` cycle is a real cycle — it clears staging on a clean semantic batch and advances the cursor — it simply cannot write memory. A cycle whose semantic batch failed keeps its staging in every mode.
