...
def promotion_mode() -> str:
    """Return ``off``, ``shadow``, or ``auto``. Anything else means ``off``.

    There is deliberately no fallback to the REM narration mode: write
    authority is its own explicit setting.
    """
    raw = os.environ.get("HERMES_DREAM_PROMOTION_MODE", "")
    normalized = raw.strip().lower()
    return normalized if normalized in PROMOTION_MODES else "off"


def _route(kind: str) -> tuple[str, str]:
    provider = os.environ.get(f"HERMES_DREAM_{kind}_PROVIDER", "").strip()
    model = os.environ.get(f"HERMES_DREAM_{kind}_MODEL", "").strip()
    if not provider or not model:
        raise SemanticRouteError(f"{kind.lower()} route is not configured")
    return provider, model


def extract_route() -> tuple[str, str]:
    """Provider/model for stage one. Never falls back to the REM route."""
    return _route("EXTRACT")


def verify_route() -> tuple[str, str]:
    """Provider/model for stage two. Never falls back to the extractor route."""
    return _route("VERIFY")


def semantic_routes() -> tuple[tuple[str, str], tuple[str, str]]:
...
def _timeout() -> float:
    """Resolve a finite, positive, bounded per-call timeout.

    Empty, malformed, non-finite, non-positive, and over-maximum values all
    resolve to :data:`DEFAULT_TIMEOUT_SECONDS`, which is the safe direction for
    a call that is promised to be bounded.
    """
    raw = os.environ.get("HERMES_DREAM_LLM_TIMEOUT", "").strip()
    if not raw:
...
def run_extraction(
    sources: list[dict[str, Any]], *, llm: Any = None
...
def run_verification(
    positives: list[dict[str, Any]], *, llm: Any = None
) -> tuple[list[dict[str, Any]], str]:
...
def assess(sources: list[dict[str, Any]], *, llm: Any = None) -> list[dict[str, Any]]:
    ...
    records, extract_runtime_model = run_extraction(sources, llm=llm)
    positives = positive_extractions(records)
    if not positives:
        return []

    verifications, verify_runtime_model = run_verification(positives, llm=llm)
