    ...
    *,
    ...
    try:
        ...
        if memory_path is None:
            ...
            memory_path = hermes_base / "memories" / "MEMORY.md"

        if promoted:
            _write_to_memory(promoted, memory_path)

        # Meta entries are recorded in DREAMS.md only. They are often notes
        # about memory hygiene itself and should not be auto-promoted into
        # either MEMORY.md or skill files without explicit review.
...
def _rem_narrative(candidates: list[dict], *, llm: Any = None) -> str:
    ...
    *llm* — optional object with a ``complete(messages, **kwargs) -> result``
    method where ``result.text`` is the response string. Typically
    ``ctx.llm`` from the plugin registration context. When *None*, or
    when ``HERMES_DREAM_REM_MODE=off``, returns a deterministic
    bullet-point summary without any LLM call. No local model or Ollama
    server is assumed.
    """
    ...
    if llm is not None and rem_mode != "off":
        try:
...
def _llm_narrative(texts: list[str], llm: Any) -> str:
    ...
    provider = os.environ.get("HERMES_DREAM_PROVIDER", "").strip() or None
    model = os.environ.get("HERMES_DREAM_MODEL", "").strip() or None
