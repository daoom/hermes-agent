...
def _record_requested_auxiliary_route(kwargs):
...
def test_auxiliary_runtime_model_collapse_fails_closed(routed, monkeypatch):
    ...
    def fake_call_llm(**kwargs):
        _record_requested_auxiliary_route(kwargs)
...
def test_auxiliary_host_fallback_route_fails_closed(
    routed, monkeypatch, fallback_stage, fallback_route
):
    from agent import auxiliary_client
    ...
    def fake_call_llm(**kwargs):
        stage = "extract" if kwargs["model"] == "dreaming-extract" else "verify"
        route_info = kwargs.get("route_info")
        if route_info is not None:
            route_info["provider"] = kwargs["provider"]
            route_info["model"] = kwargs["model"]
            if stage == fallback_stage:
                if fallback_route == "different":
                    route_info["provider"] = "openai-codex"
                    route_info["model"] = "gpt-5.6"
                else:
                    route_info["provider"] = kwargs["provider"]
                    route_info["model"] = kwargs["model"]
        calls.append((stage, route_info is not None))
        runtime_model = (
            "mistral-small-2506" if stage == "extract" else "mistral-medium-2508"
        )
        return _auxiliary_response(responses.pop(0), model=runtime_model)

    monkeypatch.setattr(auxiliary_client, "call_llm", fake_call_llm)

    with pytest.raises(ps.SemanticRouteError, match="route"):
        ps.assess(ONE_SOURCE, llm=None)

    expected_stages = ["extract"] if fallback_stage == "extract" else ["extract", "verify"]
    assert [stage for stage, _ in calls] == expected_stages
    assert all(route_info_present for _, route_info_present in calls)


@pytest.mark.parametrize("substituted_stage", ["extract", "verify"])
def test_auxiliary_single_substituted_route_fails_closed(
    routed, monkeypatch, substituted_stage
):
    from agent import auxiliary_client
    ...
    with pytest.raises(ps.SemanticRouteError, match="configured route"):
        ps.assess(ONE_SOURCE, llm=None)
...
def test_auxiliary_route_attestation_is_required(routed, monkeypatch):
    from agent import auxiliary_client
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: _auxiliary_response(
            _extraction(), model="mistral-small-2506"
        ),
    )

    with pytest.raises(ps.SemanticRouteError, match="authoritatively recorded"):
        ps.assess(ONE_SOURCE, llm=None)
...
def test_auxiliary_path_revalidates_locally_and_fails_closed(routed, monkeypatch):
    ...
    monkeypatch.setattr(auxiliary_client, "call_llm", invalid_response)

    with pytest.raises(ps.SemanticContractError):
        ps.assess(ONE_SOURCE, llm=None)


def test_auxiliary_connection_error_fails_closed_after_one_retry(routed, monkeypatch):
    from agent import auxiliary_client
    ...
    with pytest.raises(ConnectionError, match="provider transport unavailable"):
        ps.assess(ONE_SOURCE, llm=None)
...
def test_auxiliary_response_without_text_fails_closed(routed, monkeypatch):
    ...
    def empty_response(**kwargs):
        _record_requested_auxiliary_route(kwargs)
        return _auxiliary_response(None, model="mistral-small-runtime")

    monkeypatch.setattr(auxiliary_client, "call_llm", empty_response)
