...
def test_auxiliary_cycle_enforces_the_same_validators(tmp_path, semantic_routes, monkeypatch):
    ...
    def fake_call_llm(**kwargs):
        ...
        route_info["provider"] = kwargs["provider"]
        route_info["model"] = kwargs["model"]
...
def _run_auxiliary_malformed_scheduler_probe(tmp_path, monkeypatch, invalid_stage):
    """Drive one real host-validator failure through the unattended writer."""
    from agent import auxiliary_client
    ...
    calls = []
    remaining = (
        ["malformed", "valid-extraction", "valid-verification"]
        if invalid_stage == "extract"
        else ["valid-extraction", "malformed", "valid-verification"]
    )
    favorable = StructuredLlm(extract="accept", verify="accept")
    ...
    def fake_call_llm(**kwargs):
        ...
        route_info["provider"] = kwargs["provider"]
        route_info["model"] = kwargs["model"]
        response_kind = remaining.pop(0)
        if response_kind == "malformed":
            return auxiliary_client._validate_llm_response(
                SimpleNamespace(model=f"{kwargs['model']}-runtime", choices=[]),
                task="dreaming",
                provider=kwargs["provider"],
            )
        payload = json.loads(kwargs["messages"][-1]["content"])
        if response_kind == "valid-extraction":
            content = favorable._extraction(payload["candidates"])
            runtime_model = "mistral-small-2506"
        else:
            content = favorable._verification(payload["verifications_requested"])
            runtime_model = "mistral-medium-2508"
    ...
    monkeypatch.setattr(auxiliary_client, "call_llm", fake_call_llm)
    result = _schedule.dream_run(
        hermes_home=str(tmp_path), force=True, scan_recent=False, llm=None
    )
    return {
        "calls": calls,
        "remaining": remaining,
        ...
        "memory_after": _memory_digest(tmp_path),
        "staging_before": staging_before,
        "staging_after": staging.read_bytes(),
    }


def test_auxiliary_malformed_extractor_envelope_is_not_retried_into_memory_write(
    tmp_path, semantic_routes, monkeypatch
):
    """A host-invalid extractor response terminates the authorization path."""
    probe = _run_auxiliary_malformed_scheduler_probe(tmp_path, monkeypatch, "extract")

    assert probe["calls"] == ["dreaming-extract"]
    assert probe["remaining"] == ["valid-extraction", "valid-verification"]
    assert probe["result"]["promoted"] == 0
    assert probe["result"]["would_promote"] == 0
    assert probe["result"]["decisions"][0]["reason"] == "semantic_validation_unavailable"
    assert probe["memory_after"] == probe["memory_before"]
    assert probe["staging_after"] == probe["staging_before"]


def test_auxiliary_malformed_verifier_envelope_is_not_retried_into_memory_write(
    tmp_path, semantic_routes, monkeypatch
):
    """A host-invalid verifier response cannot be re-rolled into acceptance."""
    probe = _run_auxiliary_malformed_scheduler_probe(tmp_path, monkeypatch, "verify")

    assert probe["calls"] == ["dreaming-extract", "dreaming-verify"]
    assert probe["remaining"] == ["valid-verification"]
    assert probe["result"]["promoted"] == 0
    assert probe["result"]["would_promote"] == 0
    assert probe["result"]["decisions"][0]["reason"] == "semantic_validation_unavailable"
    assert probe["memory_after"] == probe["memory_before"]
...
def _run_auxiliary_route_fallback_scheduler_probe(
    tmp_path, monkeypatch, fallback_stage, *, substitute_only=False
):
    """Return favorable output through a host fallback route at one stage."""
    from agent import auxiliary_client
    ...
    def fake_call_llm(**kwargs):
        payload = json.loads(kwargs["messages"][-1]["content"])
        stage = "extract" if "candidates" in payload else "verify"
        route_info = kwargs.get("route_info")
        if route_info is not None:
            if stage != fallback_stage or not substitute_only:
                route_info["provider"] = kwargs["provider"]
                route_info["model"] = kwargs["model"]
            if stage == fallback_stage:
                route_info["provider"] = "openai-codex"
                route_info["model"] = "gpt-5.6"
        calls.append({
            "stage": stage,
            "requested": (kwargs["provider"], kwargs["model"]),
            "route_info_present": route_info is not None,
            "returned": (
                None
                if route_info is None
                else (route_info.get("provider"), route_info.get("model"))
            ),
        })
        if stage == "extract":
            content = favorable._extraction(payload["candidates"])
...
@pytest.mark.parametrize("fallback_stage", ["extract", "verify"])
def test_auxiliary_host_fallback_route_cannot_authorize_memory_write(
    tmp_path, semantic_routes, monkeypatch, fallback_stage
):
    probe = _run_auxiliary_route_fallback_scheduler_probe(
        tmp_path, monkeypatch, fallback_stage
    )

    expected_stages = ["extract"] if fallback_stage == "extract" else ["extract", "verify"]
    assert [call["stage"] for call in probe["calls"]] == expected_stages
    assert all(call["route_info_present"] for call in probe["calls"])
    assert probe["calls"][-1]["returned"] == ("openai-codex", "gpt-5.6")
    assert probe["result"]["promoted"] == 0
    assert probe["result"]["would_promote"] == 0
    assert probe["result"]["decisions"][0]["reason"] == "semantic_validation_unavailable"
    assert probe["memory_after"] == probe["memory_before"]
...
@pytest.mark.parametrize("substituted_stage", ["extract", "verify"])
def test_auxiliary_single_substituted_route_cannot_authorize_memory_write(
    tmp_path, semantic_routes, monkeypatch, substituted_stage
):
    probe = _run_auxiliary_route_fallback_scheduler_probe(
        tmp_path, monkeypatch, substituted_stage, substitute_only=True
    )

    expected_stages = (
        ["extract"] if substituted_stage == "extract" else ["extract", "verify"]
    )
    assert [call["stage"] for call in probe["calls"]] == expected_stages
    assert all(call["route_info_present"] for call in probe["calls"])
    assert probe["calls"][-1]["returned"] == ("openai-codex", "gpt-5.6")
